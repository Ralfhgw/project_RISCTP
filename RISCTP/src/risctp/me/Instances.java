...
// $Id: Instances.java,v 1.13 2024/07/12 14:11:12 schreine Exp $
//
...
public class Instances
{
   ...
   ***************************************************************************/
  public static List<Rule> generate(ProofProblem problem,
    List<Rule> rules, List<Rule> sources)
  {
  ...
  private static class GroundTermAnalyzer
  {
    ...
    {
      for (Rule rule : rules) analyze(rule);
    }
  ...
  private static class Generator
  {
    ...
    private List<Rule> result0;
    
    // the instances of all rules
    private List<Rule> result;
    public List<Rule> rules() { return result; }
    ...
    public Generator(Map<TypeSymbol,List<Object>> typeMap)
    {
      ...
      this.result0 = new ArrayList<Rule>();
     ...
     *************************************************************************/
    public List<Rule> generate(List<Rule> rules)
    {
      // add ground rules
     ...
     * @param rule the rule to be instantiated.
     *************************************************************************/
    private void instantiate(Rule rule)
    {
      ...
      Rule rule0 = new Rule(rule.name + "." + result0.size(),
          new VariableSymbol[0], lits0, rule.negs, rule.notGoals);
      result0.add(rule0);
    }
  }
}
